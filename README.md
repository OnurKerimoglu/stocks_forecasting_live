...
- Deploy the training workflow and start the worker: once the server has started and a worker pool is created (previous 2 steps), issue `make prefect_deploy_train`. Note that this command will call the [deploy_training_worfklow.py](deploy_training_workflow.py) script, which deploy the training workflow from the git repository, dev or prod branches, as set in the script. The deployed workflow can be seen in the GUI, Deployments tab, with 'Ready' status, once the worker has started (may take a few seconds)
